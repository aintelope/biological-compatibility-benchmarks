...
def set_mem_commit_limit(pid, size, min_free_swap=None, retry_count=10):
    for i in range(0, retry_count):  # assigning to job fails sometimes
        try:
            ...
            Exception
        ) as msg:  # possibly already assigned to job object? (this can be done only once)
            print(msg)
            pass
...
def set_mem_commit_limit_worker(pid, size, min_free_swap=None):
    ...
    job = ctypes.windll.kernel32.CreateJobObjectW(
        None, None
    )  # NB! CreateJobObjectW not CreateJobObjectA
    # assert job != 0
